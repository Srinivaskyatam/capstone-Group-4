...
marketing_emails.df <- marketing_emails.df %>%
    mutate(open_rate = ifelse((sent - failed) == 0, NA, (opened / (sent - failed)) * 100))

# Group by day of the week and calculate the average open rate
...
# Reorder the open_rate_by_day based on the new factor levels
open_rate_by_day <- open_rate_by_day %>%
  arrange(day_of_week)

# View the result with the custom order
print(open_rate_by_day)

# Create a bar plot for open rates by day of the week
...
view(marketing_emails.df)
# Summarize responses by day of the week
email_summary <- marketing_emails.df %>%
  group_by(day_of_week) %>%
  summarise(total_opened = sum(opened))

# Plot the results
ggplot(email_summary, aes(x = reorder(day_of_week, total_opened), y = total_opened)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Marketing Emails Responses by Day of the Week",
       x = "Day of the Week",
       y = "Number of Opened Emails") +
  theme_minimal()
...
# Plot a pie chart to show the open rate by email type
